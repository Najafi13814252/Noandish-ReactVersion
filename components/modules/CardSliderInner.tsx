...
function CardSliderInner({ courses }: { courses: courseType[] }) {
    const [isBeginning, setIsBeginning] = useState(true)
    const [isEnd, setIsEnd] = useState(false)
    return (
        <div className="relative mx-3 md:mx-4">
            {/* دکمه‌ها */}
            <section className='flex gap-2 absolute left-36 -top-11.5'>
            ...
            <Swiper className="overflow-hidden h-full" slidesPerView={4.5} slidesPerGroup={1} modules={[Navigation]}
                ...
                {
                    courses.map(course => (
                        <SwiperSlide key={course.id} className='pr-4 py-4'>
                            <Card {...course} />
                        </SwiperSlide>
                    ))
                }
...
export default CardSliderInner
